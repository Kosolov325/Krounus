...
agent_killed = (ti_on_agent_killed_or_wounded, 0, 0, [], # server and clients: handle messages, score, loot, and more after agents die
   [(store_trigger_param_1, ":dead_agent_id"),
    ...
    (try_begin),
     ...
     (player_get_slot, ":task3", ":player_id", slot_player_quest_task_3),
     (lt, ":task1", 0),
     (lt, ":task2", 0),
     (lt, ":task3", 0),
     (player_slot_gt, ":player_id", slot_player_quest_settlement, 0),
     (player_get_slot, ":faction_id", ":player_id", slot_player_faction_id),
     (try_for_players, ":players"),
           (player_is_active, ":players"),
           (try_begin),
            (player_slot_eq, ":players", slot_player_faction_id, ":faction_id"),
            (multiplayer_send_2_int_to_player, ":players", server_event_script_message_set_color, quest_error_message),
            (multiplayer_send_string_to_player, ":players", server_event_script_message, "@Your faction's caravan was failed.."),
            (multiplayer_send_2_int_to_player, ":players", server_event_script_message_set_color, script_message_color),
            (multiplayer_send_int_to_player, ":players", server_event_play_sound, "snd_quest_failed"),
           (else_try),
            (player_slot_eq, ":players", slot_player_faction_id, "fac_outlaws"),
            (str_store_player_username, s2, ":player_id"),
            (multiplayer_send_2_int_to_player, ":players", server_event_script_message_set_color, quest_color_message),
            (multiplayer_send_string_to_player, ":players", server_event_script_message, "@You sucessuful stole the caravan by player {s2}"),
            (multiplayer_send_2_int_to_player, ":players", server_event_script_message_set_color, script_message_color),
            (multiplayer_send_int_to_player, ":players", server_event_play_sound, "snd_quest_succeeded"),
           (try_end),
     (try_end),
    (try_end),
...
agent_hit = (ti_on_agent_hit, 0, 0, [], # server: apply extra scripted effects for special weapons, hitting animals, and when overloaded by armor
   [(store_trigger_param_1, ":attacked_agent_id"),
    ...
    (try_begin),
      ...
      (this_or_next|eq, reg0, "itm_sarranid_two_handed_axe_b"),
      (this_or_next|eq, reg0, "itm_two_handed_axe"),
      (this_or_next|eq, reg0, "itm_two_handed_battle_axe"),
      (this_or_next|eq, reg0, "itm_long_axe_a"),
      (this_or_next|eq, reg0, "itm_executioners_axe"),
      (eq, reg0, "itm_woodcutter_axe"),
      (store_agent_hit_points, ":life",  ":attacked_agent_id"),
      (try_begin),
       (gt, ":damage_through", ":damage_dealt"),
       (val_sub, ":life", ":damage_through"),
      (else_try),
       (val_sub, ":life", ":damage_dealt"),
      (try_end),
      (le, ":life", 0),
      (player_get_slot, ":dead_faction_id", ":attacked_id", slot_player_faction_id),
      (player_get_slot, ":killer_faction_id", ":attacker_id", slot_player_faction_id),
      (call_script, "script_cf_factions_are_hostile", ":killer_faction_id", ":dead_faction_id"),
      (player_get_slot, ":task1", ":attacker_id", slot_player_quest_task_1),
      (val_add, ":task1", 1),
      (assign, reg5, ":task1"),
      (multiplayer_send_2_int_to_player, ":attacker_id", server_event_script_message_set_color, quest_color_message),
      (multiplayer_send_string_to_player, ":attacker_id", server_event_script_message, "@You killed {reg5}/15."),
      (multiplayer_send_2_int_to_player, ":attacker_id", server_event_script_message_set_color, script_message_color),
      (player_set_slot, ":attacker_id", slot_player_quest_task_1, ":task1"),
      (ge, ":task1", 15),
      (call_script, "script_quest_completed", ":attacker_id", 0, "itm_long_axe_a", "itm_tab_shield_round_e", "itm_nordic_warlord_helmet"),
     (else_try),
      (player_slot_eq, ":attacker_id", slot_player_quest, 4),
      (player_get_slot, ":dead_faction_id", ":attacked_id", slot_player_faction_id),
      (player_get_slot, ":killer_faction_id", ":attacker_id", slot_player_faction_id),
      (call_script, "script_cf_factions_are_hostile", ":killer_faction_id", ":dead_faction_id"),
  ...
  [
          (try_for_players, ":player_id"),
           ...
           (try_begin),
             ...
             (val_add, ":hungry_time", 1),
             (agent_set_slot, ":agent_id", slot_agent_hungry_time,  ":hungry_time"),
             (try_begin),
               (eq, ":hungry_time", 180),
               (store_agent_hit_points, ":health_percent", ":agent_id", 0),
               (gt, ":health_percent", 10),
               (val_sub, ":health_percent", 10),
               (agent_set_hit_points, ":agent_id", ":health_percent", 0),
               (multiplayer_send_string_to_player, ":player_id", server_event_script_message, "@You feel very hungry..."),
               (agent_set_slot, ":agent_id", slot_agent_hungry_time, ":hungry_time", 0),
             (try_end),
           (else_try),
              ...
              (agent_set_slot, ":agent_id", slot_agent_hungry_time, ":hungry_cooldown"),
              (try_begin),
...
server_announces = (1, 0, 0, [(multiplayer_is_server),(eq,"$allow_server_messages",1),],#Custom server announcements system
       [
        (val_add,"$message_timer",1),
        (store_mul,":interval",60,"$server_message_interval"), #5 minutes
        (try_begin),
          (ge,"$message_timer",":interval"),
            (try_begin),  
             (eq,"$server_message",1), 
             (str_store_string,s1,"@Cooking, Repairing or Healing others now gives you payments."),
            (else_try),
             (eq,"$server_message",2), 
             (str_store_string,s1,"@Fishing now is way more profitable. Catch some fishes and cook them to get paid."),
            (else_try),
             (eq,"$server_message",3), 
             (str_store_string,s1,"@You can now duel on the Arena Grounds. Go to the general arena of the map, win duels, and get some payment for entertaining the public!"),
            (else_try),
             (eq,"$server_message",4), 
             (str_store_string,s1,"@Hunt wild animals with the huntsman class and get paid. If you also cook the meat, you will come out with even better profits."),
            (else_try),
             (eq,"$server_message",5), 
             (str_store_string,s1,"@Smith weaponry and armors in the local shops and repair structures. Smithing is a very high profitable path for you."),
            (else_try),
             (eq,"$server_message",6), 
             (str_store_string,s1,"@Keep your important items safe in the Item Banking. Go to the main bank of the map, usually close to the commoner spawn to access it."),
            (else_try),
             (eq,"$server_message",7), 
