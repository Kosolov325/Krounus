...
(server_add_message_to_log, "str_debug_2"),
      (agent_get_player_id, ":attacker_player_id", ":attacker_agent_id"),
(server_add_message_to_log, "str_debug_3"),
      (player_is_active, ":rider"),
(server_add_message_to_log, "str_debug_4"),
      (player_is_active, ":attacker_player_id"),  
(server_add_message_to_log, "str_debug_5"),
      (player_get_slot, ":fac_attacker", ":attacker_player_id", slot_player_faction_id),
(server_add_message_to_log, "str_debug_6"),
      (player_get_slot, ":fac_attacked", ":rider", slot_player_faction_id),
(server_add_message_to_log, "str_debug_7"),
      (eq, ":fac_attacker", ":fac_attacked"), 
(server_add_message_to_log, "str_debug_8"),
      (assign, ":fail", 0),
    (try_end),    
    (try_begin),
     (neg|is_between, reg0, scripted_items_begin, scripted_items_end),
     ...
     (player_get_slot, ":fac_attacker", ":attacker_player_id", slot_player_faction_id),
     (player_get_slot, ":fac_attacked", ":attacked_player_id", slot_player_faction_id),
     (assign, ":fail", 0),
     (try_begin),
      (eq, ":fac_attacker", "fac_commoners"),
      (eq, ":fac_attacked", "fac_commoners"),
     (else_try),
      (neq, ":attacker_player_id", ":attacked_player_id"),
      (eq, ":fac_attacker", ":fac_attacked"),
     (else_try),
      (assign, ":fail", 1),
     (try_end),
