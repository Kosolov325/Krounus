    ...
    (store_script_param, ":left", 3), # 1 makes the door rotate the other way, for matching left and right doors
     ...
     (try_end),
      ...
      (try_begin),
       ...
       (try_begin),
        (scene_prop_slot_eq, ":instance_id", slot_scene_prop_rent_room, 1), 
        (multiplayer_send_string_to_player,":player_id", server_event_script_message, "@You used rent room: {reg1}"),
       (else_try),
        (scene_prop_slot_eq, ":instance_id", slot_scene_prop_pdoor, 1), 
        (multiplayer_send_string_to_player,":player_id", server_event_script_message, "@You used house's door: {reg1}"),
       (try_end),
      (else_try),
         (eq, ":fail", 1),
         (multiplayer_send_int_to_player, ":player_id", server_event_play_sound, "snd_failure"),
