...
scripts.extend([
  ...
  [(store_script_param, s0, 1),
   ...
   (assign, ":failure", 0),
   (try_begin),
      (str_equals, s0, "@servermessage", 1),
      (try_begin),
       (player_is_admin, ":player_id"),
       (multiplayer_send_string_to_player,":player_id", server_event_script_message, "@Use '/servermessage 1 or 0' to enable/disable server announces."),
      (else_try),
        (assign, ":failure", 1),
      (try_end),
   (else_try),
      (str_starts_with, s0, "@servermessage ", 1),
        (try_begin),
           (player_is_admin, ":player_id"),
           (str_store_substring, s0, s0, 14),
           (str_to_num, ":value", s0),
             (try_begin),
               (eq, ":value", 0),
               (assign, "$allow_server_messages", 0),
               (multiplayer_send_string_to_player,":player_id", server_event_script_message, "@The server announcements have been disabled."),
             (else_try),
               (eq, ":value", 1),
               (assign, "$allow_server_messages", 1),
               (multiplayer_send_string_to_player,":player_id", server_event_script_message, "@The server announcements have been enabled."),
             (else_try),
               (assign, ":failure", 1),
               (multiplayer_send_string_to_player,":player_id", server_event_script_message, "@The value can only be 0 or 1."),
             (try_end),
        (else_try),
           (assign, ":failure", 1),
        (try_end),
   (else_try),
     (str_equals, s0, "@servermessagetimer", 1),
      (try_begin),
       (player_is_admin, ":player_id"),
       (multiplayer_send_string_to_player,":player_id", server_event_script_message, "@Use '/servermessagetimer number' to set the server messages timer (Only integer numbers)."),
       (multiplayer_send_string_to_player,":player_id", server_event_script_message, "@The maximum value is 10."),
      (else_try),
        (assign, ":failure", 1),
      (try_end),
   (else_try),
      (str_starts_with, s0, "@servermessagetimer ", 1),
      (try_begin),
           (player_is_admin, ":player_id"),
           (str_store_substring, s0, s0, 19),
           (str_to_num, ":value", s0),
             (try_begin),
               (is_between, ":value", 1, 11),
               (assign, "$server_message_interval", ":value"),
               (assign, reg10, ":value"),
               (multiplayer_send_string_to_player,":player_id", server_event_script_message, "@The server announcements timer have been changed for {reg10} minutes."),
             (else_try),
               (multiplayer_send_string_to_player,":player_id", server_event_script_message, "@ERROR:The value can only be between 1-10."),
             (try_end),
        (else_try),
           (assign, ":failure", 1),
        (try_end),
