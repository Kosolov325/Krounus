...
scripts.extend([
  ...
  ("cf_player_load_keys", #koso
   [(store_script_param, ":player_id", 1),
    (store_script_param, s2, 2),
        ...
        (assign, ":end", slot_player_pdoor_end),
        (assign, ":loop", 1),
        (try_for_range, ":slot_id", slot_player_pdoor_id, ":end"),
         (try_begin),
          (eq, ":loop", 1),
          (str_to_num, ":id", s10),
          (player_set_slot, ":player_id", ":slot_id", ":id"),
         (else_try),
          (eq, ":loop", 2),
          (str_to_num, ":id", s11),
          (player_set_slot, ":player_id", ":slot_id", ":id"),
        ...
        (assign, ":loop", 1),
        (assign, ":slot_id", 0),
        (try_for_range, ":slot_id", slot_player_pdoor_id_ow, ":end"),
        (try_begin),
         (eq, ":loop", 1),
         (str_to_num, ":id", s14),
         (player_set_slot, ":player_id", ":slot_id", ":id"),
        (else_try),
         (eq, ":loop", 2),
         (str_to_num, ":slot_id", s15),
         (player_set_slot, ":player_id", ":slot_id", ":id"),
        (else_try),
         (eq, ":loop", 3),
         (str_to_num, ":slot_id", s16),
         (player_set_slot, ":player_id", ":slot_id", ":id"),
        (else_try),
         (eq, ":loop", 4),
         (str_to_num, ":slot_id", s17),
         (player_set_slot, ":player_id", ":slot_id", ":id"),
          (assign, ":end", slot_player_pdoor_id_ow),
        (try_end),
        (val_add, ":loop", 1),
  ...
  [(store_script_param, ":instance_id", 1),
    ...
    (scene_prop_get_slot, ":inventory_count", ":instance_id", slot_scene_prop_inventory_count),
    (store_add, ":inventory_end", ":inventory_count", slot_scene_prop_inventory_begin),
    (str_regex_get_matches, ":amount", s1, s0, "str_regex", 20),
